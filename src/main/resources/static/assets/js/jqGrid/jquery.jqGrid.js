	...
	styleUI : {
		jQueryUI : {
			...
			inlinedit : {
				...
				icon_add_nav : "ace-icon fa fa-plus-circle purple",
			...
			navigator : {
				...
				icon_view_nav : "ace-icon fa fa-search-plus grey",
			...
			subgrid : {
				icon_plus : 'ace-icon fa fa-plus-circle purple',
			...
			fmatter : {
				...
				icon_add : "ace-icon fa fa-plus-circle purple",
				icon_save : "ui-icon-disk",
				icon_cancel : "ui-icon-cancel",
		...
		return this.each(function(){
			...
			if(!$t.p.multiselect) {	
				...
				if (  ia === -1 ){
					...
					//新增记录后，selarrrow[0]将为空，导致即便单选一条记录仍然为空的bug
					if($t.p.selarrrow.length == 2 && $t.p.selarrrow[0] == "") {
						tpsr = $t.p.selarrrow[1];
					}
					else {
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		if(!p.zIndex) {
			...
			if(parentZ) {
				...
				p.zIndex =Math.max.apply(null, $.map($('body  *'), function(e,n){
		...
		return this.each(function(){
			...
			$.jgrid.viewModal("#"+$.jgrid.jqID(IDs.themodal),{
				...
				onShow : function(h) {
					...
					o.alertleft = (o.alertleft/2 - parseInt($(h.w).width(),10)/2) + document.body.scrollLeft;
					o.alerttop = (o.alerttop/2- parseInt($(h.w).height(),10)/2) + document.body.scrollTop;
					$(h.w).css("top", o.alerttop);
					$(h.w).css("left", o.alertleft);
				},
				onHide :  function(h) {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.edit) {
					...
					.click(function(){
						...
						//selarrrow返回的数组size中会包含一条空记录，即便单选length也为2。
						if (ids.length > 2 || (ids.length == 2 && ids[0] != "")) {
							FA.utils.alert("请选择一条数据进行编辑。");
						    return false;
						}
						if (!$(this).hasClass( commonstyle.disabled )) {
							var sr = $t.p.selrow;
